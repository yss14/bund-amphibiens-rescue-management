...
const isType = <T extends TypeNames>(name: T, value: unknown): value is TypeNameType<T> =>
	typeof (value) === name;
...
export type CheckerInput<M> =
	M extends Checker<infer A, infer B> ? A :
	never;
export type CheckerSuccess<M> =
	M extends Checker<infer A, infer B> ? B :
	never;
...
export const Type = <T extends TypeNames>(name: T): Checker<unknown, TypeNameType<T>> =>
...
export const KeysPartial = <T>(schema: KeysSchema<T>): Checker<Partial<Record<keyof T, unknown>>, { [key in keyof T]: T[key] }> => {
...
export const Or = <T extends unknown[]>(...types: { [key in keyof T]: Checker<unknown, T[key]> }): Checker<unknown, T[number]> => {
	return value => {
		const errors: string[][] = [];
...
export const TypeArray = (value: unknown): Check<unknown[]> => Array.isArray(value) ? [null, value] : [["expected an array"]];
export const TypeCheck = <U, T extends U>(check: ((value: U) => value is T)) => (value: U): Check<T> => check(value) ? [null, value] : [["expected custom type"]];
export const TypeEnum = <T>(_enum: T) => <Checker<unknown, T[keyof T]>>OneOf(...Object.values(_enum).filter((x): x is number => typeof (x) === "number"));
export const TypeEnumString = <T>(_enum: T) => <Checker<unknown, T[keyof T]>>OneOf(...Object.values(_enum).filter((x): x is number => typeof (x) === "string"));
